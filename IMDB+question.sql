...
-- Country = 20, worlwide_gross_income = 3724, languages = 194 and production_company columns = 528 have NULL values.
...
-- 4265 Drama movies shown after processing the above query.
...
-- 3289 movies shown in one genre from above query.
...
-- Movies in Action genre have longer duration than other genres.
...
WHERE movie_rank<=10;
 ...
 -- yes, favourite movie FAN in the top 10 movies with an average rating of 9.6.
...
-- Dream Warrior Pictures or National Theatre Live production have produced the most number of hit movies according to above query.
...
-- Top 3 movies belong to drama genre
-- The Brighton Miracle has highest average rating of 9.5
-- There are 8 movies found which begins with "The" in the title name.
...
-- 361 Movie_Count with median_rating = 8.
...
-- yes German language shows highest Vote than Italian Votes.


-- Another apporach by  VOTES_SUMMARY/language
WITH VOTES_SUM AS 
(
...
LANGUAGE_VOTES AS
(
SELECT languages FROM VOTES_SUM
ORDER BY VOTES DESC
LIMIT 1
)
...
top_movie_genre
WHERE g.genre in (top_movie_genre.genre) AND avg_rating>8
...
-- James Mangold , Joe Russo and Anthony Russo are top three directors
...
-- Top 2 actors are Mammootty and Mohanlal shown from above query.
...
-- Top three production houses by their movies are Marvel Studios, Twentieth Century Fox and Warner Bros.
...
-- Top actor is Vijay Sethupathi with total votes of 23114 have movie count = 5 , avg = 8.42 with acor rank 1.
...
-- Total votes is 2269 with movie count 3.
...
-- highest avg movies duration is of Action Genre followed by Adventure and comedy.
...
-- Top 3 genre are Drama in 2017 , Movie names = Shatamanam Bhavati, winner , Thank you for your service.
...
--  Top 3 actresses based on Super Hit movies are Parvathy Thiruvothu, Susan Brown and Amanda Lawrence as per above query analysis.
...
-- Top 9 directors according to above Analysis
-- Andrew jones
-- A.L. Vijay
-- Ozgur Bakar
-- Justice Price
